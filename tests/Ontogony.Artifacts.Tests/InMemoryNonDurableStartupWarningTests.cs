...
public sealed class InMemoryNonDurableStartupWarningTests
{
    [Fact]
    public async Task In_memory_artifact_store_logs_warning_when_not_development()
    {
    ...
    [Fact]
    public async Task In_memory_artifact_store_does_not_warn_in_development()
    {
        var sink = new ConcurrentQueue<(LogLevel Level, string Message)>();
        var builder = Host.CreateApplicationBuilder();
