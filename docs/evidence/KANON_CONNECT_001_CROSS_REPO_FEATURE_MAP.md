...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **Ontology versions** | `GET …/ontologies/{id}/versions`; `POST …/ontologies/validate`; `POST …/ontology-versions/{id}/publish`; `DELETE …/ontology-versions/{id}` | `/kanon/ontologies` — `OntologyVersionsPage` | `useKanonOntologyVersions`; `kanonClient.listKanonOntologyVersions`; `useValidateKanonOntology` / `usePublishKanonOntologyVersion` / `useDeleteKanonOntologyVersion` | `kanon.baseUrl`, `kanon.ontologyId`, `kanon.serviceToken` | **Consumer:** run `ontologyVersionId` on start/resume. **Link:** `kanonOntologyVersionHref` in `buildAllagmaSemanticAuthorityLinkSlots` | — | Indirect via `ontologyVersionId` on run nodes in `resolveEvidenceSpine` | `kanon-api` Postgres seed; CORS origins; no Kanon-specific compose key beyond service image | `OntologyV0RouteInventoryTests`; FE `OntologyVersionsPage.test.tsx`; Playwright 014 step 2 | Default FE `ontologyVersionId` = `gaming-core-v0` vs seed `gaming-core@0.1.0` → **KANON-CONNECT-002** |
...
| **Conexus assistance** | `GET …/conexus/assistance/capability`; `POST …/assistance/*` (5 draft endpoints) | `/kanon/assistance` — `ConexusAssistancePage`, `KanonConexusAssistanceWorkbench` | `kanonConexusAssistanceClient`; `useKanonConexusAssistanceCapability`, `useInvokeKanonConexusAssistance` | `kanon.baseUrl`, `kanon.serviceToken`, `kanon.ontologyVersionId`, `allagma.defaultActorId`, `allagma.defaultActorRoles` | — | **Consumer:** Kanon posts `v1/chat/completions` with project API key; **FE link:** `modelInvocationId` → `/conexus/observability?modelCallId=` | Conexus model-call graph in spine; Kanon assistance decision via `kanonDecisionId` | `Kanon__ConexusAssistance__Enabled`, `__BaseUrl`, `__ProjectApiKey`; shares `CONEXUS_PROJECT_API_KEY` | `ConexusAssistanceApiTests`, `ConexusAssistanceNonAuthorityInvariantTests`; Playwright 014 steps 8, 11–12 | `modelInvocationId` drilldown coverage partial in cross-service tests → **KANON-CONNECT-004** |
| **Semantic graph** | `GET …/semantic-graph` | `/kanon/decisions?…&semanticGraph=1` — `KanonSemanticEvidenceGraphPanel` | `useKanonSemanticGraph`; `getKanonSemanticGraph` | `kanon.baseUrl`, `kanon.serviceToken` | **Link:** `kanonSemanticGraphHref` from Allagma semantic authority card | — | **Not imported** in `resolveEvidenceSpineDeepGraphs` (decision/trace only) → **KANON-CONNECT-005** | — | `SemanticGraphApiTests`; Playwright 014 step 6 | Evidence Spine workbench does not auto-merge semantic-graph nodes |
...
| --- | --- | --- | --- |
| Frontend default `ontologyVersionId` (`gaming-core-v0`) ≠ Docker seed (`gaming-core@0.1.0`) | Medium | **KANON-CONNECT-002** | ENV-SEED overrides at runtime; defaults still drift in fresh settings |
| Allagma run GET lacks `domainPackId`; semantic link slot documents unavailable | Low | **KANON-CONNECT-003** | Links work when metadata present |
| No automated cross-link matrix test for every Allagma artifact → Kanon route | Medium | **KANON-CONNECT-003** | Building blocks exist (`buildAllagmaSemanticAuthorityLinkSlots`) |
| Conexus assistance `modelInvocationId` observability drilldown not fully proven cross-service | Low | **KANON-CONNECT-004** | FE adapter links when id returned |
| Evidence Spine does not import Kanon `GET /semantic-graph` | Medium | **KANON-CONNECT-005** | Graph panel works on Kanon decisions page |
| Route/catalog drift gate not enforced in CI for connect phase | Low | **KANON-CONNECT-006** | Inventories exist; gate is follow-up |
