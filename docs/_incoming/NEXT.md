...
**2026-05-24 update:** `SETTINGS-SECURITY-UX-001` implemented (settings/security UX: credential taxonomy, warning consolidation, actor presets, diagnostics privacy, execution posture on settings). **`EVAL-EVIDENCE-QUALITY-001` implemented** (evaluations dashboard evidence-aware: metadata coverage, row evidence quality, current-page summary basis, comparison absence reasons). **`RELEASE-READINESS-TRUTH-001` implemented** (release readiness page: artifact vs RC posture, freshness, route classification, caveats, related evidence links). **Next:** `RUNTIME-LOCK-CI-GOVERNED-E2E-001`.
