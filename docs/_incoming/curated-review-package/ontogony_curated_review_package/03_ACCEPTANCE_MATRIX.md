...
| ALLAGMA-EVIDENCE-001 | closed-source | Audit/eval export bundles include `downstreamReferences`; cohesion smoke validates Kanon decision + Conexus model-call locators. Evidence: `allagma-dotnet/docs/evidence/ALLAGMA_EVIDENCE_001_EVIDENCE.md`. |
| ALLAGMA-STREAM-001 | closed-source | Non-default local smoke streams through Conexus and emits redacted Allagma progress events; unit/API PASS; live stack operator-run. Evidence: `allagma-dotnet/docs/evidence/ALLAGMA_STREAM_001_EVIDENCE.md`. |
...
| CONEXUS-RETENTION-001 | closed-with-caveat | Admin retention status/run APIs; dry-run/execute with counts only; tests prove expired record handling. `lastRun` is process-local (clears on restart). Evidence: `conexus-dotnet/docs/evidence/CONEXUS_RETENTION_001_EVIDENCE.md`. |
| CONEXUS-RETENTION-002 | planned | Durable Postgres maintenance-run history; multi-instance `lastRun` story; restart-survival tests. Not a Sprint 4 blocker. |
...
| KANON-CONEXUS-ASSIST-001 | closed-source | Disabled/mock/local paths; redacted `draft_only` decision + provenance; non-authority invariants. Evidence: `kanon-dotnet/docs/evidence/KANON_CONEXUS_ASSIST_001_EVIDENCE.md`. |
| KANON-DOMAINPACK-GOV-001 | closed-source | Domain-pack lifecycle docs and tests cover promote, active lookup, deprecate, blocked promotion, hash mismatch, and signature failure. Evidence: `kanon-dotnet/docs/reviews/KANON_DOMAINPACK_GOV_001_VALIDATION_REPORT.md`. |
| SYSTEM-SPRINT4-STATUS-RECON-001 | closed | Platform sprint plan and acceptance matrix mark Sprint 4 source/evidence closed; service evidence cross-linked; runtime promotion remains pending `SYSTEM-ALPHA-004`. Evidence: `ontogony-platform/docs/evidence/SYSTEM_SPRINT4_STATUS_RECON_001_EVIDENCE.md`. |
