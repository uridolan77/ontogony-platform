...
5. `KANON-POSTGRES-LOCAL-001` — prove durable Kanon local path.
6. `KANON-EVIDENCE-001` — evidence packs for major semantic workflows.
7. `CONEXUS-TOOLS-001` — tool/function pass-through.
8. `CONEXUS-GOV-DRILLDOWN-001` — model-call/usage drill-down contracts.
9. `ALLAGMA-TOOL-TRUST-001` — real-tool trust model, without enabling real execution.
10. `ALLAGMA-SANDBOX-OBS-001` — explicit simulated/local execution labels.

## Sprint 2 — Frontend operator truth

11. `FE-CLEANUP-001` — resolve duplicate config artifacts.
12. `FE-AUTH-001` — route-level operator auth guard.
13. `FE-ERRBOUND-001` — route-level error boundaries.
14. `FE-FIXTURE-MATRIX-001` — live/fallback/fixture matrix.
15. `FE-STUBS-001` — honest states or live wiring for thin pages.
16. `FE-API-ADAPTER-001` — adapter contract tests.
