...
- Single PostgreSQL container with five logical databases (not five Postgres instances).
...
- **Host port collisions:** local dev APIs can shadow Docker port mappings on Windows when probes use `localhost`. Stop local processes on **5081–5085**, **5175**, or override ports in `docker/local-working-system/.env`. See `docs/evidence/ENV_COMPOSE_001_DOCKER_COMPOSE_ORCHESTRATION_EVIDENCE.md` (port-collision note).
...
- ENV-PG-001 used port `55432` and database `allagma_e2e`; Docker program uses host-mapped Postgres (default **55433** in `.env.example`) and five logical DBs inside the container.
