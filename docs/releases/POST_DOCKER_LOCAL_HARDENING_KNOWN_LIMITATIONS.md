...
**Date:** 2026-05-19  
**Scope:** `POST-DOCKER-HARDENING-CLOSEOUT-001` (twelve hardening PRs + `CI-COST-001`)
...
### Frontend check sprawl (local)

`ontogony-frontend` `npm run check` runs many sequential gates (`fixtures:check`, `replay:check`, `config:check`, OpenAPI, routes, build, tests, etc.). Valuable for local quality; **CI** now path-scopes heavy jobs (`CI-COST-001`). Local `npm run check` duration is unchanged.
...
- Full `ontogony-ui` `npm run check:full` (Storybook, size limits) is heavier than the packaging subset used in integration evidence.
...
- `CI-COST-001` is **done**; Actions usage is reduced on scoped PRs.
- **Branch protection is not configured by this package.** When you enable it, use aggregate checks only (`ci-complete` / `check-full`). Do **not** require scoped jobs that skip on docs-only PRs.
- Manual workflows (`system-cohesion`, `restart-e2e`) do not run on every merge by design.
